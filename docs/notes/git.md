...
- **MIT 许可证**: 允许任何人使用、复制、修改、合并、发布、分发、再许可和/或销售软件的副本，但必须保留原作者的版权声明和 MIT 协议文本。

- **Apache 许可证 2.0**: 允许用户使用、修改和分发软件，但要求保留版权声明和许可证文本，并且在分发修改后的软件时必须注明所做的更改。还提供了专利授权。

- **BSD 许可证**: 有两种主要版本，2-Clause BSD 和 3-Clause BSD。它们允许用户使用、修改和分发软件，但要求保留版权声明和许可证文本。3-Clause BSD 还禁止使用原作者的名字进行推广。

- **GNU 通用公共许可证 (GPL)**: 允许用户自由使用、修改和分发软件，但要求任何基于 GPL 许可的软件的衍生作品也必须以 GPL 许可发布（即“传染性”）。

  **LGPL** 是 GPL 的“弱化版”，允许商业使用，但要求对 LGPL 许可的库进行修改时，那部分代码必须以 LGPL 许可发布。

随便用 → MIT，想要保护代码开源 → Apache 2.0，想要强制开源 → GPL，可以用，但别改我 → LGPL。
