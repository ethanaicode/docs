...
公司服务器偶尔出问题，老板让我看看。看了下nginx错误日志，发现有很多路径扫描，而且用的都是同一个IP，想着浪费服务器资源，顺手写了个工具，扫描最近120秒出现10次错误的 IP 并直接用 iptables 封禁。

观察了下没问题就直接上线了。

结果早上整个服务器上的业务都中断了，熟悉的502，本能的去找PHP的原因，之前重启PHP都能临时恢复，这次重启也失效了，就很奇怪。

于是总结最近做了什么修改，突然想到了这个改动。

我们服务之间会用127.0.0.1进行通信，如果这个IP被封禁，我们服务就会无法正常通信，导致整个服务阻塞而无法提供正常的服务。

查了下封禁的规则，发现确实封禁了 127.0.0.1，赶紧移除这个规则，关掉了封禁工具，整个服务才恢复正常。

该错误导致服务中止了接近3小时，错过了早上高峰期，老板虽然没多说什么，但是感觉还挺自责的，确实给公司带来不小的损失。

> 教训：在操作防火墙规则时，一定要小心，避免误封关键的本地通信 IP。
...
- **AZW3**：是随着 2011 年亚马逊推出 Kindle Fire 平板电脑时一同推出的。
