...
打开文件后，看到红色的标题，显示有错误，但有时候标红的地方不明显，该如何定位呢？

- `View` --> `Problems` ，这里会列出所有错误（文件名 + 行号 + 错误信息）。

- 通过快捷键 `F8`，会跳转到下一个错误，`Shift + F8` 会跳转到上一个错误。
...
- **分列以空格分割的内容或者数字**
  ...
  希望将其整理成“内容”和“数字”分开、适合放入表格的结构，在 Excel 中，我们可以使用 `数据` --> `分列` 功能来将以空格分割的内容或者数字分列。

  1. 选中需要分列的单元格

  2. 点击 `数据` --> `分列`

  3. 在弹出的对话框中，选择 `分隔符号`，然后选择 `空格`
