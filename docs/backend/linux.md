...
- **pwd**: 显示目前的目录
  ...
  `pwd -P` 显示物理路径（不包含符号链接，可以获取到真实路径）
...
- **du**: 查看文件大小
  ...
  **推荐用法**: `du -shd 1 .` 显示当前目录下的文件和目录的大小
...
- **find**: 查找文件
  ...
  如: `-mtime 7` 表示 7 天前修改的文件（注意: `+7` 表示 7 天<u>前之前</u>修改的文件，`-7` 表示 7 天<u>前之内</u>修改的文件）
...
- **userdel**: 删除用户
  ...
  `-r` 删除用户的同时删除用户的家目录和邮件池（没有邮件池会报错，但不影响删除）
...
# 搜索历史命令
!string  # 执行历史命令中最后一条以 string 开头的命令

# 清空历史命令
history -c
# 将历史命令写入历史文件（默认是 ~/.bash_history）
# 如果修改了历史命令文件，需要执行这个命令来让修改生效
history -w
...
# 查找某段时间内修改过的文件
#   修改时间晚于 2026-05-01 00:00:00 且早于 2026-05-31 23:59:59 的文件
find . -type f -newermt "2026-05-01 00:00:00" ! -newermt "2026-05-31 23:59:59"

# 定时任务自动删除昨天的日志文件
0 2 * * * find /path/to/logs -type f ! -newermt "$(date +\%Y-\%m-\%d\ 00:00:00)" -delete > /dev/null 2>&1
```
...
- `.tar.gz` 或 `.tgz`: 使用 gzip 压缩
  ...
  `gz` 后缀只是“压缩容器”，如果没有 `.tar`，无法表示这是一个归档文件（恢复文件的集合）。
...
> 下面的命令会创建一个以当前日期和时间命名的压缩文件，包含指定的文件或目录。

```bash
# 压缩多个文件
tar -czvf archive_$(date +'%Y%m%d_%H%M%S').tar.gz file1 file2 file3
# 压缩目录
...
- `create 0640 www-data adm`: 创建新日志文件，权限设置为 0640，所有者为 www-data，所属组为 adm。
  ...
  `adm` 表示 `Administrator` 在Linux系统中是一个专门用于管理日志文件的用户组，通常具有读取日志文件的权限，但没有修改权限。

  这样设置可以确保日志文件的安全性，同时允许管理员查看日志内容。
...
```bash
awk '$7 ~ /^\/f\?/' /var/log/nginx/access.log | wc -l
```
...
- `ssh -Tv git@github.com`: 显示详细的连接过程，便于调试
