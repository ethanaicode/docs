...
sudo apt update
# 安装特定版本的 PHP 及常用扩展
sudo apt install php7.4-cli php7.4-fpm php7.4-mysql php7.4-curl php7.4-json php7.4-mbstring php7.4-xml php7.4-zip
    # 豪华版安装（包含更多常用扩展）
    sudo apt install php7.4-cli php7.4-fpm php7.4-mysql php7.4-curl php7.4-json php7.4-mbstring php7.4-xml php7.4-zip \
php7.4-gd php7.4-bcmath php7.4-redis php7.4-fileinfo php7.4-opcache php7.4-apcu php7.4-intl php7.4-gettext php7.4-ldap php7.4-soap
# 查看 PHP 版本
php -v
...
- 配置如果不在上面常见目录下，也会在 `/etc/` 下，以 `php` 开头的文件或者目录。
...
可以通过设置 `pm.status_path` 的值，来显示 PHP-FPM 的状态页面，默认值为 `/status`，然后在 Nginx 的配置文件中，添加一个 location 来访问这个状态页面：
...
之后就可以通过 `http://yourdomain.com/status` 来访问 PHP-FPM 的状态页面了，类似下面的内容：
...
total processes:      69
# 历史上 最高并发 worker 使用数
max active processes: 41
# 历史上 最高等待 worker 处理的请求数
